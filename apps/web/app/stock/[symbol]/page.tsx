...
function StockDetail({ symbol }: { symbol: string }) {
  ...
  if (quoteError) return <main className="page"><Link className="back" href="/">← Back to market</Link><StateMessage tone="error" title="Instrument unavailable" detail={`We could not load ${symbol}. Check the symbol and try again.`} /></main>;
  if (!quote) return <main className="page"><Link className="back" href="/">← Back to market</Link><p>Loading stock data...</p></main>;

  const up = quote.change >= 0;
  return <div className="shell"><header className="topbar"><div className="brand"><span className="brand-mark">M</span>Market Watch</div><div className="status"><span className={`dot ${offline ? "down" : ""}`} />{offline ? "Backend offline" : marketStatus?.open ? "Live updates" : "Market closed · last update shown"}</div></header><main className="page"><Link className="back" href="/">← Back to market</Link>{offline ? <StateMessage tone="error" title="Backend offline" detail="The displayed quote is the last available snapshot." /> : !marketStatus?.open ? <StateMessage tone="closed" title="Market closed" detail={`Last update: ${new Date(quote.timestamp).toLocaleString("en-IN")}`} /> : null}<div className="detail-header"><div><div className="eyebrow">{quote.exchange} · {symbol}</div><h1>{symbol}</h1><div className="muted">Market data snapshot</div></div><div><div className="detail-price">{quote.price.toLocaleString("en-IN", { maximumFractionDigits: 2 })}</div><div className={`change ${up ? "up" : "down"}`}>{up ? "+" : ""}{quote.change.toFixed(2)} ({up ? "+" : ""}{quote.changePercent.toFixed(2)}%)</div></div></div><section className="panel chart-wrap"><div className="panel-heading"><div><div className="eyebrow">Price action</div><h2>{range} candles</h2></div><div className="timeframes">{["1D", "1W", "1M", "1Y"].map((item) => <button className={range === item ? "active" : ""} key={item} onClick={() => setRange(item)}>{item}</button>)}</div></div>{historicalError ? <StateMessage tone="error" title="Historical data unavailable" detail="The quote is available, but chart history could not be loaded." /> : <StockChart candles={candles} tick={tick} />}</section><section className="stats">{[["Open", quote.open], ["Day high", quote.dayHigh], ["Day low", quote.dayLow], ["Prev close", quote.previousClose], ["52W high", quote.week52High], ["52W low", quote.week52Low], ["Volume", quote.volume.toLocaleString("en-IN")], ["Last update", new Date(quote.timestamp).toLocaleTimeString("en-IN")]].map(([label, value]) => <div className="stat" key={String(label)}><div className="stat-label">{label}</div><div className="stat-value">{typeof value === "number" ? value.toLocaleString("en-IN", { maximumFractionDigits: 2 }) : value}</div></div>)}</section></main></div>;
}
